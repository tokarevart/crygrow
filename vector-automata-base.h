...
class vector_automata_base : automata_base<N, Cell> {
public:
    Cell* get_cell(const spt::vec<N, std::int64_t>& pos) const override final {
    ...
    void reset_cell(const spt::vec<N, std::int64_t>& pos, Cell* ptr = nullptr) override final {
        auto actualpos = actual_pos(pos);
    ...
    virtual void resize_cells(const spt::vec<N, std::int64_t>& cuboid_corner0,
                              const spt::vec<N, std::int64_t>& cuboid_corner1) final {
        ...
        spt::vec<N, std::size_t> new_dims_lens = far_corner - new_origin;
        spt::vec<N, std::int64_t> dorigin = m_origin - new_origin;
    ...
    virtual void reserve_cells(const spt::vec<N, std::int64_t>& cuboid_corner0,
                               const spt::vec<N, std::int64_t>& cuboid_corner1) final {
        ...
        //spt::vec<N, std::size_t> new_dims_lens = far_corner - new_origin;
        //spt::vec<N, std::size_t> dorigin = m_origin - new_origin;
        //
        //std::vector<std::unique_ptr<Cell>> new_cells(
        //    std::accumulate(new_dims_lens.x.begin(), new_dims_lens.x.end(), 1,
        //                    std::multiplies<std::size_t>()));
...
private:
    spt::vec<N, std::int64_t> m_origin;
    spt::vec<N, std::size_t> m_dims_lens;
    std::vector<std::unique_ptr<Cell>> m_cells;

    std::size_t offset(const spt::vec<N, std::size_t>& pos) const {
        return offset(m_dims_lens);
    }
    std::size_t offset(const spt::vec<N, std::size_t>& pos, 
                       const spt::vec<N, std::size_t>& dims_lens) const {
    ...
    spt::vec<N, std::int64_t> actual_pos(const spt::vec<N, std::int64_t>& pos) const {
        return actual_pos(pos, m_origin);
    }
    spt::vec<N, std::int64_t> actual_pos(const spt::vec<N, std::int64_t>& pos,
                                         const spt::vec<N, std::int64_t>& origin) const {
        return pos - origin;
    }
    spt::vec<N, std::size_t> actual_pos(std::size_t offset) const {
        return actual_pos(offset, m_dims_lens);
    }
    spt::vec<N, std::size_t> actual_pos(std::size_t offset,
                                        const spt::vec<N, std::size_t>& dims_lens) const {
        // implement
    }
};
...
namespace gridimpl {
template <std::size_t N, typename Cell>
using vector = vector_automata_base<N, Cell>;
}
